...
def _line_panel(
    ...
    *,
    ...
    out = [f'<text x="{_W/2:.0f}" y="{oy+20}" text-anchor="middle" '
    ...
    for t in _nice_ticks(ymin, ymax):
        ...
        out.append(f'<text x="{px0-8}" y="{y+4:.1f}" text-anchor="end" '
                   f'font-size="11" fill="#374151">{t:g}</text>')
    # x ticks (at the actual multiples)
    for xv in x_vals:
        x = sx(xv)
        out.append(f'<line x1="{x:.1f}" y1="{py1}" x2="{x:.1f}" y2="{py1+5}" '
                   f'stroke="#9ca3af" stroke-width="1"/>')
        out.append(f'<text x="{x:.1f}" y="{py1+18}" text-anchor="middle" '
    ...
    out.append(f'<text x="{px0-46}" y="{(py0+py1)/2:.0f}" text-anchor="middle" '
               f'font-size="12" fill="#111827" transform="rotate(-90 {px0-46} '
               f'{(py0+py1)/2:.0f})">{_esc(y_label)}</text>')
    out.append(f'<text x="{(px0+px1)/2:.0f}" y="{py1+38}" text-anchor="middle" '
               f'font-size="12" fill="#111827">{_esc(x_label)}</text>')

    # series
    for idx, (label, ys, units) in enumerate(series):
        color = _COLORS[idx % len(_COLORS)]
        pts = " ".join(f"{sx(x):.1f},{sy(y):.1f}" for x, y in zip(x_vals, ys))
        ...
        ly = py0 + 6 + idx * 18
        out.append(f'<line x1="{px1+12}" y1="{ly}" x2="{px1+30}" y2="{ly}" '
                   f'stroke="{color}" stroke-width="3"/>')
        out.append(f'<text x="{px1+34}" y="{ly+4}" font-size="11" '
                   f'fill="#111827">{_esc(label)}</text>')
    return "\n".join(out)


def _wrap(height: int, body: str) -> str:
    return (f'<svg xmlns="http://www.w3.org/2000/svg" width="{_W}" height="{height}" '
...
def ownership_and_roi_svg(result: SimResult) -> str:
    ...
    p1 = _line_panel(
        0, "Your % ownership as market cap grows", x,
        [("ownership %", own, "%")], "market-cap multiple (log)", "ownership %")
    p2 = _line_panel(
        _PH, "Return multiple as market cap grows", x,
        [("sell-back (redeem)", redeem, "x"), ("settlement win", settle, "x")],
        "market-cap multiple (log)", "value / spend  (x)")
...
def mc_histogram_svg(mc: McResult, bins: int = 40) -> str:
    ...
    out = [f'<text x="{_W/2:.0f}" y="20" text-anchor="middle" font-size="15" '
    ...
    for i, c in enumerate(counts):
        ...
        bw = (px1 - px0) / bins
        bh = (c / cmax) * (py1 - py0)
        out.append(f'<rect x="{x:.1f}" y="{py1-bh:.1f}" width="{bw-1:.1f}" '
                   f'height="{bh:.1f}" fill="#93c5fd"/>')
    out.append(f'<line x1="{px0}" y1="{py1}" x2="{px1}" y2="{py1}" stroke="#374151"/>')
    # markers
    for val, color, lab in [(1.0, "#374151", "break-even"),
                            (mc.median_settle, "#059669", "median"),
                            (mc.mean_settle, "#dc2626", "mean")]:
        if lo <= val <= hi:
            x = sx(val)
            out.append(f'<line x1="{x:.1f}" y1="{py0}" x2="{x:.1f}" y2="{py1}" '
                       f'stroke="{color}" stroke-width="1.5" stroke-dasharray="4 3"/>')
            out.append(f'<text x="{x:.1f}" y="{py0-2}" text-anchor="middle" '
                       f'font-size="10" fill="{color}">{_esc(lab)} {val:.2f}x</text>')
    for t in _nice_ticks(lo, hi):
        x = sx(t)
        out.append(f'<text x="{x:.1f}" y="{py1+16}" text-anchor="middle" '
                   f'font-size="10" fill="#374151">{t:g}x</text>')
    out.append(f'<text x="{(px0+px1)/2:.0f}" y="{py1+34}" text-anchor="middle" '
